...
/** @brief LED Brightness Characteristic UUID. */
#define BT_UUID_LSS_FADE_VAL BT_UUID_128_ENCODE(0x00000003, 0x665a, 0x46ec, 0xa6eb, 0x36715013ce98)
...
#define BT_UUID_LSS_FADE BT_UUID_DECLARE_128(BT_UUID_LSS_FADE_VAL)

/** @brief Callback type for when the application wants to fade the color. */
typedef void (*fade_cb_t)(const led_rgbw *color, uint32_t duration);
...
struct my_lss_cb {
	fade_cb_t fade_cb;                // Fade color callback
